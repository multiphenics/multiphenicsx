...
def DecorateGetEigenPair(BlockSLEPcEigenSolver):
    from multiphenics.block.function import BlockFunction # avoid recursive imports
    ...
    class DecoratedBlockSLEPcEigenSolver(BlockSLEPcEigenSolver):
        def get_eigenpair(self, i=0, r_vec=None, c_vec=None):
            ...
            if isinstance(r_vec, BlockFunction):
                r_vec.block_vector().set_local(r_vec_out.array())
...
def BlockSLEPcEigenSolver(A, B=None, bcs=None):
    from multiphenics.block.fem import BlockDirichletBC # avoid recursive imports
