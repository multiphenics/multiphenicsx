...
// This file is temporarily included in block_ext until its modifications
...
#include <block/mesh/SubDomain.h>

using namespace dolfin;

//-----------------------------------------------------------------------------
CustomizedSubDomain::CustomizedSubDomain(const double map_tol) : SubDomain(map_tol),
                                             _geometric_dimension(0)
{
  // Do nothing
}
//-----------------------------------------------------------------------------
CustomizedSubDomain::~CustomizedSubDomain()
{
  // Do nothing
}
//-----------------------------------------------------------------------------
void CustomizedSubDomain::mark_cells(Mesh& mesh,
...
//-----------------------------------------------------------------------------
void CustomizedSubDomain::mark_facets(Mesh& mesh,
                            std::size_t sub_domain,
                            bool check_midpoint) const
{
...
//-----------------------------------------------------------------------------
void CustomizedSubDomain::mark(Mesh& mesh,
                     std::size_t dim,
                     std::size_t sub_domain,
                     bool check_midpoint) const
{
  //dolfin_assert(mesh.domains().markers(dim));
  //dolfin_error("Not yet updated (CustomizedSubDomain::mark) ");
  apply_markers(mesh.domains().markers(dim), dim, sub_domain, mesh,
                check_midpoint);
}
//-----------------------------------------------------------------------------
void CustomizedSubDomain::mark(MeshFunction<std::size_t>& sub_domains,
                     std::size_t sub_domain,
                     bool check_midpoint) const
{
...
//-----------------------------------------------------------------------------
void CustomizedSubDomain::mark(MeshFunction<int>& sub_domains,
                     int sub_domain,
                     bool check_midpoint) const
{
  apply_markers(sub_domains, sub_domain, *sub_domains.mesh(), check_midpoint);
...
//-----------------------------------------------------------------------------
void CustomizedSubDomain::mark(MeshFunction<double>& sub_domains,
                     double sub_domain,
                     bool check_midpoint) const
{
  apply_markers(sub_domains, sub_domain, *sub_domains.mesh(), check_midpoint);
...
//-----------------------------------------------------------------------------
void CustomizedSubDomain::mark(MeshFunction<bool>& sub_domains,
                     bool sub_domain,
                     bool check_midpoint) const
{
  apply_markers(sub_domains, sub_domain, *sub_domains.mesh(), check_midpoint);
...
//-----------------------------------------------------------------------------
void CustomizedSubDomain::mark(MeshValueCollection<std::size_t>& sub_domains,
                     std::size_t sub_domain,
                     const Mesh& mesh,
                     bool check_midpoint) const
{
...
//-----------------------------------------------------------------------------
void CustomizedSubDomain::mark(MeshValueCollection<int>& sub_domains,
                     int sub_domain,
                     const Mesh& mesh,
                     bool check_midpoint) const
{
...
//-----------------------------------------------------------------------------
void CustomizedSubDomain::mark(MeshValueCollection<double>& sub_domains,
                     double sub_domain,
                     const Mesh& mesh,
                     bool check_midpoint) const
{
...
//-----------------------------------------------------------------------------
void CustomizedSubDomain::mark(MeshValueCollection<bool>& sub_domains,
                     bool sub_domain,
                     const Mesh& mesh,
                     bool check_midpoint) const
{
...
//-----------------------------------------------------------------------------
std::size_t CustomizedSubDomain::geometric_dimension() const
{
...
void CustomizedSubDomain::apply_markers(S& sub_domains,
                              T sub_domain,
                              const Mesh& mesh,
                              bool check_midpoint) const
{
...
//-----------------------------------------------------------------------------
template<typename T>
void CustomizedSubDomain::apply_markers(std::map<std::size_t, std::size_t>& sub_domains,
                              std::size_t dim,
                              T sub_domain,
                              const Mesh& mesh,
                              bool check_midpoint) const
