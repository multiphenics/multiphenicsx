...
class Stokes(SaddlePointProblem):
    ...
    def __init__(self, block_V, **kwargs):
        ...
        SaddlePointProblem.__init__(self, block_V, **kwargs)
        # ... and also store FEniCS data structures for assembly
        assert "subdomains" in kwargs
        assert "boundaries" in kwargs
        self.subdomains, self.boundaries = kwargs["subdomains"], kwargs["boundaries"]
        block_up = BlockTrialFunction(block_V)
        (self.u, self.p) = block_split(block_up)
        block_vq = BlockTestFunction(block_V)
        (self.v, self.q) = block_split(block_vq)
        self.s = self.u
        self.r = self.v
    ...
    def assemble_operator(self, term):
        ...
        if term == "a":
            u = self.u
            v = self.v
            a0 = [[(u[0].dx(0)*v[0].dx(0) + u[1].dx(0)*v[1].dx(0))*dx(1), 0], [0, 0]]
            a1 = [[(u[0].dx(0)*v[0].dx(1) + u[0].dx(1)*v[0].dx(0) + u[1].dx(0)*v[1].dx(1) + u[1].dx(1)*v[1].dx(0))*dx(1), 0], [0, 0]]
            a2 = [[(u[0].dx(1)*v[0].dx(1) + u[1].dx(1)*v[1].dx(1))*dx(1), 0], [0, 0]]
            a3 = [[(u[0].dx(0)*v[0].dx(0) + u[1].dx(0)*v[1].dx(0))*dx(2), 0], [0, 0]]
            a4 = [[(u[0].dx(1)*v[0].dx(1) + u[1].dx(1)*v[1].dx(1))*dx(2), 0], [0, 0]]
            a5 = [[(u[0].dx(0)*v[0].dx(0) + u[1].dx(0)*v[1].dx(0))*dx(3), 0], [0, 0]]
            a6 = [[(u[0].dx(1)*v[0].dx(1) + u[1].dx(1)*v[1].dx(1))*dx(3), 0], [0, 0]]
            a7 = [[(u[0].dx(0)*v[0].dx(0) + u[1].dx(0)*v[1].dx(0))*dx(4), 0], [0, 0]]
            a8 = [[(u[0].dx(1)*v[0].dx(1) + u[1].dx(1)*v[1].dx(1))*dx(4), 0], [0, 0]]
            return (a0, a1, a2, a3, a4, a5, a6, a7, a8)
        elif term == "b":
            u = self.u
            q = self.q
            b0 = [[0, 0], [- q*u[0].dx(0)*dx(1), 0]]
            b1 = [[0, 0], [- q*u[0].dx(1)*dx(1), 0]]
            b2 = [[0, 0], [- q*u[1].dx(1)*dx(1), 0]]
            b3 = [[0, 0], [- q*u[0].dx(0)*dx(2), 0]]
            b4 = [[0, 0], [- q*u[1].dx(1)*dx(2), 0]]
            b5 = [[0, 0], [- q*u[0].dx(0)*dx(3), 0]]
            b6 = [[0, 0], [- q*u[1].dx(1)*dx(3), 0]]
            b7 = [[0, 0], [- q*u[0].dx(0)*dx(4), 0]]
            b8 = [[0, 0], [- q*u[1].dx(1)*dx(4), 0]]
            ...
            v = self.v
            f0 = [inner(self.f, v)*dx(0), 0]
            f1 = [inner(self.f, v)*dx(1), 0]
            f2 = [inner(self.f, v)*dx(2), 0]
            f3 = [inner(self.f, v)*dx(3), 0]
            return (f0, f1, f2, f3)
        elif term == "g":
            q = self.q
            g0 = [0, self.g*q*dx(0)]
            ...
            if term == "dirichlet_bc_u":
                bc0 = BlockDirichletBC([bc0, None])
                return (bc0,)
            elif term == "dirichlet_bc_s":
                bc0 = BlockDirichletBC([bc0])
                return (bc0,)
...
element = BlockElement(element_u, element_p)
block_V = BlockFunctionSpace(mesh, element, components=[["u", "s"], "p"])
